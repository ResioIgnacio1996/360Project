...
export class ImportacionProgramacion implements OnInit {
  ...
  importar():void{
    if(!this.version.trim()||!this.fechaInicioProgramacion||this.errores.length)return;this.procesando=true;
    this.service.importar(this.proyectoId,this.version,this.fechaInicioProgramacion,this.datos).subscribe({
      ...
      error:e=>{this.mensaje=[e?.error?.message,e?.error?.error].filter(Boolean).join(': ')||'No se pudo importar';this.errores=e?.error?.errores||[];this.procesando=false}
    });
  }
