...
import { ClientePayload } from '../../../shared/interfaces/cliente.interface';
...
export class ClienteForm implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (id) { this.idCliente = id; this.esEdicion = true; this.cargarCliente(id); }
  }

  cargarCliente(id: number): void {
    this.cargando = true;
    this.clienteService.getCliente(id).subscribe({
      next: cliente => { this.clienteForm.patchValue(cliente); this.cargando = false; },
      error: err => { this.error = err?.error?.message || 'Error al cargar cliente'; this.cargando = false; }
    });
  }
  ...
  guardar(): void {
    ...
    this.guardando = true;
    this.error = '';
    const data = this.clienteForm.getRawValue() as ClientePayload;
    const request = this.esEdicion && this.idCliente
      ? this.clienteService.actualizarCliente(this.idCliente, data)
      : this.clienteService.crearCliente(data);
    request.subscribe({
      next: () => this.router.navigate(['/clientes']),
      error: err => { this.error = err?.error?.message || 'Error al guardar cliente'; this.guardando = false; }
    });
  }
  ...
  formatearCuil(event: Event): void {
    ...
    this.clienteForm.get('cuil')?.setValue(resultado, { emitEvent: false });
  }
}
