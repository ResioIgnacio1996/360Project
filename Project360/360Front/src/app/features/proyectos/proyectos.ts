...
<<<<<<< HEAD
  ...
  opcionesUbicacion(indice: number, pais = 'TODOS', provincia = 'TODOS'): string[] {
    const valores = this.proyectos
      .filter(p => {
        const u = this.ubicacion(p.direccion);
        return (pais === 'TODOS' || u[0] === pais) && (provincia === 'TODOS' || u[1] === provincia);
      })
  ...
  filtrar(): void {
    ...
    this.filtrados = this.proyectos.filter(p => {
      ...
      return (!q || texto.includes(q))
        && (this.clienteId === 'TODOS' || Number(p.cliente_id) === Number(this.clienteId))
        ...
        && this.enRango(p.fecha_inicio, this.fechaCreacionDesde, this.fechaCreacionHasta)
        && this.enRango(p.fecha_fin_estimada, this.entregaDesde, this.entregaHasta);
    });
  }
  ...
  eliminar(p: any): void {
    this.uiDialog.confirmar({
      title: 'Eliminar proyecto',
      message: `Eliminar el proyecto ${p.nombre}?`,
      confirmText: 'Eliminar',
      tone: 'danger'
    }).subscribe(confirmado => {
      if (!confirmado) return;
      this.service.eliminarProyecto(p.proyecto_id).subscribe({
        next: () => this.cargar(),
...
>>>>>>> parent of 1441af5 (cambios en db y estetica)
}
